...
export function ArtistProfile({ address, handle, bio, works, sales, path }: Props) {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-6 py-14 md:px-10 md:py-20">
      ...
      <section className="mt-12">
        <h2 className="mb-6 font-mono text-[11px] uppercase tracking-[0.25em] text-[var(--smoke)]">
          {t("artistProfile.worksTitle")} · {works.length}
        </h2>
        {works.length === 0 ? (
          <p className="font-mono text-[11px] text-[var(--smoke)]">
            {t("artistProfile.worksEmpty")}
          </p>
        ) : (
              ...
              >
                ...
                <div className="flex items-baseline justify-between gap-2 px-3 py-2.5">
                  ...
                  <span className="shrink-0 font-mono text-[10px] text-[var(--blue)]">
                    {(work.royaltyBps / 100).toFixed(0)}% {t("artistProfile.royaltyBadge")}
                  </span>
                </div>
              </Link>
            ))}
      ...
      {/* ── Recent sales — public chain facts only ── */}
      <section className="mt-14">
        <h2 className="mb-6 font-mono text-[11px] uppercase tracking-[0.25em] text-[var(--smoke)]">
          {t("artistProfile.salesTitle")}
        </h2>
        {sales.length === 0 ? (
          ...
          <ul className="divide-y divide-[var(--ember)] border-y border-[var(--ember)]">
            {sales.map((sale) => (
              <li key={sale.txHash} className="flex flex-wrap items-baseline gap-x-6 gap-y-1 py-3">
                <Link
