...
export function TokenCard({ token_id, token_uri, artist, royalty_bps, price }: TokenCardProps) {
  ...
  return (
    ...
    >
      {/* Media container — a neutral gallery mat so any aspect ratio sits inside a
          consistent frame (object-contain) without cropping the work. */}
      <div className="tc-image flex-1 bg-[var(--ember)] relative overflow-hidden -mx-7 -mt-7 p-5">
        {loading ? (
      ...
      <div className="tc-body pt-5 flex flex-col">
        ...
        <div className="tc-title font-[family-name:var(--font-display)] text-base font-bold text-[var(--offwhite)] tracking-tight leading-normal truncate">
          {title}
        </div>
        ...
        <div className="tc-footer mt-4 pt-4 border-t border-white/12 flex justify-between items-center">
          <div className="flex flex-col">
            {price && (
              <div className="tc-price font-[family-name:var(--font-mono)] text-sm font-bold text-[var(--offwhite)]">
                {price} XLM
              </div>
            )}
            <div className="tc-royalty font-[family-name:var(--font-mono)] text-[9px] tracking-[0.15em] uppercase text-[var(--smoke)] mt-0.5">
