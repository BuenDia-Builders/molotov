...
async function getCursor(): Promise<{ lastLedger: number; lastCursor: string | null }> {
  ...
  return {
    lastLedger: (data as { last_ledger: number; last_cursor: string | null }).last_ledger,
    lastCursor: (data as { last_ledger: number; last_cursor: string | null }).last_cursor,
  };
}
...
async function fetchTokenUri(tokenId: number): Promise<string> {
  try {
    ...
    const account = new Account("GANXCETUVUUILGJPVEZWM7EH66IZM5OICUPMNUWNXKIBRK425MUKZERM", "0");
